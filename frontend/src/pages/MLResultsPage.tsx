...
export default function MLResultsPage() {
  const { getAgentOutput, currentRun, chartsData } = useStore()
  ...
  const designerOutput = useMemo(() => getAgentOutput('designer') || {}, [getAgentOutput])
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(value: any, name: any) => [`${value}`, 'Importance']}
                    />
                ...
                {modelsEvaluated.map((model: any, idx: number) => (
                  <div key={idx} className="space-y-1">
                    <div className="flex justify-between text-sm">
                      <span className="font-medium">{model.name || model.model || `Model ${idx + 1}`}</span>
                      <span className="text-muted-foreground">R2: {model.r2 ?? 'N/A'}</span>
                    </div>
                    <Progress value={Math.max(0, Math.min(100, (model.r2 || 0) * 100))} className="h-2" />
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>RMSE: {model.rmse ?? 'N/A'}</span>
                      <span>{model.reason || model.suitable_for || ''}</span>
                    </div>
                  </div>
                ))}
              </div>
